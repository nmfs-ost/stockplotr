...
test_that("rda file made when indicated", {
  ...
  # expect that both figures dir and the catch_comp_figure.rda file exist
  expect_true(dir.exists(fs::path(getwd(), "figures")))
  expect_true(file.exists(fs::path(getwd(), "figures", "catch_comp_figure.rda")))
...
test_that("plot_catch_comp generates error with scaling <1", {
  # expect error
  expect_error(
    plot_catch_comp(
      out_new,
...
test_that("plot_catch_comp generates error when catch label is not found", {
...
test_that("year class lines are plotted", {
  ...
  has_lines_layer <- function(p) {
    # Get the layer information from the plot
    layers <- p$layers

    # Check if any layer's geom is a "GeomLine"
    for (layer in layers) {
  ...
  expect_true(has_lines_layer(plot))
})
