...
NEXT STEPS:
-Choose a table function as pilot (bnc?) after we start revamping those fxns
-Add args to create_rda() and export_rda() (in utils_rda.R) to allow latex-based tables to be exported
-Edit pilot table fxn to export latex-based table in rda

In asar:
-Edit create_tables_doc() to allow user to add latex-based table
-Will have to allow caption to be added from captions_alttext csv
  -Or maybe make new fxn that creates the table, then instructs the user to delete the chunk in the tables doc and add in new Latex chunk?