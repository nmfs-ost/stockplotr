    ...
    data <- data |>
      ...
      dplyr::mutate(
        ...
        estimate_lower = dplyr::case_when(
          ...
          grepl("sd", tolower(uncertainty_label)) | grepl("std", tolower(uncertainty_label)) ~ (estimate - (1.96 * uncertainty)) / scale_amount,
          grepl("cv", tolower(uncertainty_label)) ~ (estimate - ((uncertainty / 100) * estimate)) / scale_amount,
          TRUE ~ NA
        ),
