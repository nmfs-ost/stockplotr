...
#' @param unit_label String. Abbreviated spr units
#'
#' Default: "mt"
...
#' in this ordered list: "spr_weight",  "spr_numbers", "spr_expected",
#' "spr_predicted", "spr".
#'
#' Default: NULL
#'
...
#'
#' @examples
#' table_spr(stockplotr::example_data)
#'
#' table_spr(
#'   stockplotr::example_data,
#'   unit_label = "spr label",
#'   group = "fleet"
#' )
table_spr <- function(
  dat,
  ...
  if (nrow(prepared_data) == 0) {
    cli::cli_abort("No spr data found.")
  }
  ...
  # TODO: add check if there is a spr column for every error column -- if not remove the error (can keep spr)

  # merge error and spr columns and rename
  df_list <- merge_error(
    table_data,
    id_col_vals,
    unit_label,
  ...
  if (make_rda == TRUE) {
    if (length(df_list) == 1) {
      ...
      spr.units <- unit_label

      # calculate & export key quantities
      export_kqs(spr.units)

      # Add key quantities to captions/alt text
      insert_kqs(spr.units)
